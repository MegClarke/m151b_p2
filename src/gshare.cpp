...
GShare::GShare(uint32_t BTB_size, uint32_t BHR_size)
    : BTB_(BTB_size, BTB_entry_t{false, 0x0, 0x0}) // meagan: I initialized this struct to be {valid, target, tag} not sure if thats correct
      ,
      PHT_((1 << BHR_size), 0x0), BHR_(0x0), BTB_shift_(log2ceil(BTB_size)), BTB_mask_(BTB_size - 1), BHR_mask_((1 << BHR_size) - 1), TAG_mask_((1 << (30 - BTB_shift_)) - 1)
{
  //--
}
...
uint32_t GShare::predict(uint32_t PC)
{
  ...
  uint32_t PHT_index = ((PC >> 2) ^ BHR_) & BHR_mask_; // xor the PC with the BHR to get the index into the PHT
  ...
  {                                   // if the BTB says we should branch   added a tag check to ensure is the right branch based off of PC
